...
def eval_PyTorch(model, data_generator, Ks):
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        rate_batch, u_emb, i_emb = model.predict(user_batch)
    ...
    return result, u_emb, i_emb


def early_stopping(log_value, best_value, stopping_step, expected_order='recall', flag_step=100):
    assert expected_order in ['ndcg', 'recall']

    if (expected_order == 'ndcg' and log_value >= best_value) or (expected_order == 'recall' and log_value >= best_value):
        stopping_step = 0
