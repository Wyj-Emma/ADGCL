...
class Data(object):
    def __init__(self, args):
        ...
        try:
            ...
            with open(test_file, 'rb') as f:
                test_mat = pickle.load(f)  # sp_coo格式
    ...
    def create_adj_mat(self):
        ...
        adj_mat = sp.coo_matrix((np.ones(len(new_rows)), (new_rows, new_cols)),
                                shape=[self.n_users + self.n_items, self.n_users + self.n_items]).tocsr().tocoo()
            ...
            for j in range(n):
                while True:
                    ...
                    if self.R[user, neg_item] == 1:
                        ...
                        break
            train_data.append([user, pos_item])
            train_data_neg.append(neg_items)
        self.train_data = np.array(train_data)
        self.train_data_neg = np.array(train_data_neg)
    ...
    def mini_batch(self, batch_idx):
        ...
        batch_data_neg = self.train_data_neg[st:ed]
        users = batch_data[:, 0]
        pos_items = batch_data[:, 1]
        neg_items = batch_data_neg[:]
    ...
    def print_statistics(self):
        ...
        self.n_train, self.n_test, (self.n_train) / (self.n_users * self.n_items)))
        print('num_cpu_cores=%d' % num_cpu_cores)

    def get_statistics(self):
        sta = ""
        sta += 'n_users=%d, n_items=%d\t' % (self.n_users, self.n_items)
        sta += 'n_interactions=%d\t' % (self.n_train + self.n_test)
        sta += 'n_train=%d, n_test=%d, sparsity=%.5f' % (
        self.n_train, self.n_test, (self.n_train) / (self.n_users * self.n_items))
        return sta
